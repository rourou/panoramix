...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const recup = {
            ...
            walkingMonster: await clash.getClan('P0LLLVPR').then((res) => { return res.data })
        }
        ...
        const embed_2 = new MessageEmbed()
            .setColor('#ffffff')
            .setTitle(`CLAN 1 : WALKING MONSTER (${recup.walkingMonster.members}/50)`)
            .setURL('https://link.clashofclans.com/fr?action=OpenClanProfile&tag=P0LLLVPR')
            .setDescription('Clan école de la famille')
            .setThumbnail(recup.walkingMonster.badgeUrls.medium)
            .addFields(
                { name: 'Tag ', value: '#P0LLLVPR' },
                { name: 'Lien', value: 'https://link.clashofclans.com/fr?action=OpenClanProfile&tag=P0LLLVPR', inline: true },
                { name: 'Description', value: recup.walkingMonster.description }
            )

        const embed_3 = new MessageEmbed()
            .setColor('#ffffff')
            .setTitle(`CLAN 2 : L'ARMÉE (${recup.armee.members}/50)`)
        ...
        const embed_4 = new MessageEmbed()
            .setColor('#ffffff')
            .setTitle(`CLAN 3 : ASTERIX TEAM (${recup.asterix.members}/50)`)
